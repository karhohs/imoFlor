...
Sname = cell(length(dirCon),1);
% if MDA is used for smFISH then the stage position is always recorded as
% one. The following code expects the positions to have unique numbers, so
% the following for loops identify unique names and converts them into
% unique numbers.
for i = 1:length(dirCon)
    Sname{i}=regexp(dirCon(i).name,'^.+(?=_w\d+)','match','once'); %Find the stage position number
    ind = strcmp(Sname{i},Sname);
    S(i) = find(ind,1,'first');
end
Sunique = unique(S)';
Sunique(Sunique==1) = [];
Sunique2 = 1:length(Sunique);
for i = 1:length(Sunique)
    S(S==Sunique(i)) = Sunique2(i);
end
for i = 1:length(dirCon)
...
waitbar(0, hwbar, 'Finding the names of the wavelengths...');
...
waitbar(0, hwbar, 'Calculating the maximum number of z positions...');
numberZposMax = 1;
hwbarcounter = 0;
        ...
        for k = Tunique
            if ~isempty(FileNames{k,i,j}) %FileNames(Time,Position,Channel)
                ...
                for h = 1:numberOfTIFFdir
                    ...
                    labelText = regexp(FileNames{k,i,j},'^.+(?=_w)','match','once');
